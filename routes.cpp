#include "crow_all.h"
crow::response greet(){
    crow::response res;    res.code = 200;
    res.set_header("Content-Type", "text/plain");    res.body = "Hello world!";
        return res;
...
{    auto input = crow::json::load(req.body);
    ...
    oss << "{\"result\":" << result << "}";
    // Return the response with status code 200 and JSON content    crow::response res;
...
crow::response subtract(const crow::request &req){
    ...
    int result = first - second;
    std::ostringstream oss;    oss << "{\"result\":" << result << "}";
    // Return the response with status code 200 and JSON content
    crow::response res;    res.code = 200;
    res.set_header("Content-Type", "application/json");    res.body = oss.str();
    return res;
}
